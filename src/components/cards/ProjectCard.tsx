...
export function ProjectCard({ project }: Readonly<{ project: Project }>) {
	...
	return (
		<div className={`project-card relative ${external ? "external" : ""}`}>
			{external ? (
				...
				<div className="absolute bottom-2 left-2 w-full py-1 px-1 z-100">
					<p className="text-xs text-gray-300 flex items-center gap-1 bg-black/85 w-fit py-1 px-1 rounded-lg">
						<PostIcon size={20} aria-label="Internal Post" />{" "}
						<span className="sr-only">Internal Post</span>
					</p>
				</div>
			)}
			{imageUrl && (
				<div className="project-card-image flex items-center justify-center w-full">
					<Suspense
						fallback={
							<div className="w-44 h-44 bg-gray-500 rounded-md animate-pulse" />
						}
					>
						<ImageComponent
							image={{
			...
			{preview && <p className="project-card-body">{preview}</p>}
			<p className="project-card-footer">
				Published by <span className="project-card-by-name">{author}</span> on{" "}
				<span className="date">
