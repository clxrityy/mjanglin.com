...
export async function getWeather(): Promise<WeatherData | undefined> {
    try {
        ...
        if (current && hourly) {
            const data: WeatherData = {
                temp: current.variables(0)?.value() ?? CONSTANTS.DEFAULT_WEATHER_TEMP,
            ...
            return {
                temp: CONSTANTS.DEFAULT_WEATHER_TEMP,
                rain: new Float32Array()
            }
        }
    } catch (error) {
...
export async function WeatherCard({ location, country, countryCode }: Readonly<{ location: string, country: string, countryCode: string }>) {

    try {
        const weatherData = await getWeather();

        if (!weatherData) {
            return <div className="text-red-500">Weather data unavailable</div>;
        }

        const { temp, rain } = weatherData;
        ...
        return (
            <div className={`card ${exo.className}`}>
                ...
                <div className="cloud">
                    ...
                    {rain[0] > 0.478 && <BsCloudRain size={64} />}
                </div>
                <p className="main-text text-shadow">{temp.toFixed(0)}°F</p>
                <Image
                    src={`${URLS.API.flags_api}/${countryCode}/flat/32.png`}
                    alt={`${country} flag`}
                    width={50}
                    height={50}
                />
