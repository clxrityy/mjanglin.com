...
type VerifyDiscordRequestResult = | { isValid: false } | { isValid: true, interaction: InteractionData | APIPingInteraction | APIChatInputApplicationCommandInteraction };
...
export async function verifyInteractionRequest(
    ...
    return {
        interaction: JSON.parse(rawBody)as APIChatInputApplicationCommandInteraction | APIPingInteraction | InteractionData,
        isValid: true
    }
}